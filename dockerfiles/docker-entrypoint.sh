...
if [ "$1" = 'pgpool' ]; then
	...
	echo "wd_hostname = '"${HOSTNAME}"'" >> /etc/pgpool.conf
	...
	do
	    ...
	    echo "other_pgpool_hostname${idx} = '$base_hostname-$i'" >> /etc/pgpool.conf
	    echo "other_pgpool_port${idx} = ${PGPOOL_PORT}" >> /etc/pgpool.conf
	    echo "other_wd_port${idx} = ${PGPOOL_WD_PORT}" >> /etc/pgpool.conf
	    echo "heartbeat_destination${idx} = '$base_hostname-$i'" >> /etc/pgpool.conf
	    idx=$((idx+1))	
	done	
    fi
